...
exports.trade = function(trade){
	...
	if(trade.timestamp - times[0] > sauce.smoothing){
		...
		state.price_average = money.round.fiat(state.price_average);
		state.price_short_average = money.round.fiat(state.price_short_average);
		...
		state.orders = money.round.crypto(state.orders);

		//finish and write/return
		const ready = ptools.ready();
		peaks.Peak(ready);
		const items = tools.getItems(Trade);
		if(!ready) items.trade.noready = true;
		if(ready) {
			checkEmergency(Trade);
			decay.set();
		}
...
function checkEmergency(trade){
	if(typeof state.triggers.emergency === "undefined") state.triggers.emergency = false;
	let prev = state.triggers.emergency;
	if(!state.triggers.emergency && state.first_trade){		
		let emergency = state.trade_dir === "sell"?
			((trade.timestamp - state.last_profit_time - sauce.emergency.cut) > 0 && 
			(trade.price - state.price_target)*-1 > sauce.emergency.limit)||
			(trade.price - state.price_target)*-1 > sauce.emergency.limit && state.inclineshort < sauce.triggers.price*-1
			:			
			((trade.timestamp - state.last_profit_time - sauce.emergency.cut) > 0 && 
			trade.price - state.price_target > sauce.emergency.limit)||
			trade.price - state.price_target > sauce.emergency.limit && state.inclineshort > sauce.triggers.price;
	...
	if(prev!=="new" && state.first_trade && state.triggers.emergency){		
		state.trade_dir === "sell"?
			trade.price > (state.prev_price_target - sauce.emergency.limit)?eReset():null:
			trade.price < (state.prev_price_target + sauce.emergency.limit)?eReset():null;
	}
}
function eReset(){
