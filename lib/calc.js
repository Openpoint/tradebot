...
function transact(that,callback){
	...
	if(e && state.last_price){
		if(e) Log.info(`[${time.datestamp(state.triggers.emergency||that.trade.timestamp).log}] ________________ EMERGENCY -${(diff<0?diff*-1:diff).toFiat()}`);
	}
