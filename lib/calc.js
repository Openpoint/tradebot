...
function goDev(This,callback){
	...
	Log.info(
		...
		`${money.string.fiat(trade.price)} --- $${money.string.fiat(wallet.fiat?wallet.fiat:wallet.coin*trade.price)}`,
		` | BTC:${money.string.crypto(wallet.coin?wallet.coin:wallet.fiat/trade.price)}`,
		`[peaked:${state.triggered.peaked} good:${state.triggered.good} [${state.triggered.total}] glut:${state.triggered.glut} total:${state.triggered.total}]`,
		state.message||""
	);
	money.expense(v);
