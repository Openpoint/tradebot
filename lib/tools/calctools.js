	...
	*/
	if(state.first_trade){
		...
		if(emergency){
			...
			console.log(state.trade_dir,(state.price_short_average - state.price_target)*-1,vars.cut_limit,(state.price_short_average - state.price_target)*-1 > vars.cut_limit)
...
exports.sentiment = function(rates,t,trade,i,length){
	if(t){
		...
		if(state.first_trade && state.glut){
			state.price_target_adjusted = state.price_target;
			/*
			state.price_target_adjusted = state.price_target+State.bull_bear
