...
module.exports.addTrade = function(trade){
	...
	if(!trade) return;
	/*
	Bitstamp.public.ticker('btcusd',(err,data)=>{
		console.log(tools.timestamp(trade.timestamp),':',state.trade_dir+' |',trade.price);
		if(err){
			console.log(err);
			return;
		}
		...
		console.log(tools.timestamp(ts),':',data)
		console.log('')
	})
	*/
	...
	//if(state.emergency) console.log(state.trade_dir,state.price_incline)
	if(emergency || check.init(trade)){
		if(state.doldrum||state.glut||emergency){
			console.log(tools.timestamp(trade.timestamp)+' Doldrum:'+state.doldrum+' Glut:'+state.glut,' Emergency:'+emergency);			
		}
