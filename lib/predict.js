...
Check.prototype.bull_bear = function(trade){
	...
	state.bull_bear > limits.bull_bear[0] && peaked():
	state.bull_bear < limits.bull_bear[0]*-1 && peaked()
...
Check.prototype.inertia = function(trade){
	...
	*/
	if(inertia){
		...
		state.trade_dir === 'sell'?
			console.log(peak.bull_bear, peak.bull_bear - state.bull_bear,limits.bull_bear[1]):
			console.log(peak.bull_bear, state.bull_bear -peak.bull_bear, limits.bull_bear[1])
...
Check.prototype.frenzy = function(trade){
	const frenzy =  state.trade_dir === 'sell'?
		state.rate_frenzy >= limits.frenzy[0] && peaked():
		state.rate_frenzy <= limits.frenzy[0]*-1  && peaked()
	if(frenzy){
		return true;
	}
