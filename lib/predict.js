...
function Check(){
	...
	this.init = function(trade){
		...
		Glut(trade);
		good = peaked();
		if(good && this[stages[0]](trade)){
			good = [
				this[stages[1]](trade),
				this[stages[2]](trade),
				this[stages[3]](trade)
			];
			console.log(tools.timestamp(trade.timestamp),good)
			good = good.filter((foo)=>{return foo}).length > 1;
		}else{
			good = false;
		}			
...
Check.prototype.bull_bear = function(trade){
	...
	state.bull_bear > limits.bull_bear[0] && prev.bull_bear > 0:
	state.bull_bear < limits.bull_bear[0]*-1 && prev.bull_bear < 0
...
Check.prototype.inertia = function(trade){
	...
	state.trade_inertia > limits.inertia[0]:
	state.trade_inertia < limits.inertia[0]
	/*
	const inertia = state.trade_dir === 'sell'?
...
Check.prototype.frenzy = function(trade){
	const frenzy =  state.trade_dir === 'sell'?
		state.rate_frenzy >= limits.frenzy[0] /*&& prev.frenzy > 0*/:
		state.rate_frenzy <= limits.frenzy[0]*-1 /*&& prev.frenzy < 0*/
	if(frenzy){
		return true;
	}
...
function Peak(trade){
	return {
		...
		frenzy:state.trade_dir === "sell"?
			state.rate_frenzy > limits.frenzy[0]?(state.rate_frenzy >= (peak.frenzy||state.rate_frenzy)?state.rate_frenzy:peak.frenzy):0:
			state.rate_frenzy < limits.frenzy[0]*-1?(state.rate_frenzy <= (peak.frenzy||state.rate_frenzy)?state.rate_frenzy:peak.frenzy):0,
...
function peaked(){
	...
	let limit = state.glut?2:1;
	return state.trade_dir === "sell"?
		sell.filter((foo)=>{return foo}).length > limit:
		buy.filter((foo)=>{return foo}).length > limit
		/*
		(peak.inertia > 0 && peak.inertia - state.trade_inertia > limits.inertia[1]) && (
...
function Glut(trade){
	state.glut = state.trade_dir === 'sell'?
